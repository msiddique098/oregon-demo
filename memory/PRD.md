# Royal Crypto Rewards — Product Requirements Document

## Original Problem Statement
Build a modern, premium full-stack crypto rewards platform with a luxury "royal" UI/UX. The platform is NOT a real crypto exchange — all balances, rewards, profits, commissions, withdrawals and task progress are fully controlled manually by the admin from the admin panel. No real-time crypto API, live blockchain data, or auto-generated earnings should be used.
...
- **Color palette:** Black + Gold + Royal Purple (luxury fintech)
- **Default admin:** `admin@royalcrypto.com` / `Admin@123`
- **Demo user:** `user@royalcrypto.com` / `User@123`
...
## User Personas
1. **Royal Subject (user):** Earns visual rewards, completes tasks, refers friends, withdraws.
2. **Royal Admin (super_admin):** Full control — manually edits all user metrics, manages packages, approves withdrawals, runs bulk campaigns, curates live feed.
...
- 5 default packages (Basic→Royal VIP)
...
## Test Suite
`/app/backend/tests/test_royal_crypto_api.py` — 24 backend pytest cases, all passing.
