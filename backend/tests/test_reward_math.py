...
def test_signup_spins_are_two_randomized_rewards_between_9_01_and_19_99():
    import random
    from reward_math import build_signup_spin_rewards

    queue = build_signup_spin_rewards(rng=random.Random(42))
    assert len(queue) == 2
    assert all(9.01 <= value <= 19.99 for value in queue)


def test_signup_spins_can_differ_between_users_without_fixed_total():
    import random
    from reward_math import build_signup_spin_rewards
