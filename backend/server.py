...
MARKET_CACHE_SECONDS = int(os.environ.get("MARKET_CACHE_SECONDS", "8"))
...
async def get_market_snapshot(limit: int = TRADING_DEFAULT_LIMIT, vs_currency: str = "usd", include_custom: bool = True, force: bool = False) -> dict:
    ...
    error = None
    try:
        binance_live = await asyncio.to_thread(_fetch_binance_markets_sync)
        try:
            coingecko_live = await asyncio.to_thread(_fetch_markets_sync, limit, vs_currency)
            binance_symbols = {normalize_symbol(item.get("symbol")) for item in binance_live}
            combined = list(binance_live) + [item for item in coingecko_live if normalize_symbol(item.get("symbol")) not in binance_symbols]
            coins = merge_markets(combined, include_custom=include_custom)
            source = "binance+coingecko"
        except Exception as cg_exc:
            error = f"CoinGecko: {str(cg_exc)[:160]}"
            coins = merge_markets(binance_live, include_custom=include_custom)
            source = "binance"
    except Exception as binance_exc:
        error = f"Binance: {str(binance_exc)[:160]}"
        try:
    ...
    visible = coins[:limit + (4 if include_custom else 0)]
    provider = {
        "binance+coingecko": "Binance + CoinGecko",
        "binance": "Binance",
        "coingecko": "CoinGecko",
    }.get(source, "Fallback cache")
    payload = {
        "coins": visible,
        "count": len(visible),
...
def _ohlc_step(timeframe: str) -> int:
...
def _fetch_binance_ticker_price_sync(pair: str) -> float:
    """Fetch the latest exchange price for a supported Binance spot pair.

    This is used only for exchange-supported pairs. It prevents chart candles
    from being moved by internal animated/fallback market prices.
    """
    symbol = _pair_to_binance_symbol(pair)
    response = requests.get(
    ...
    if price <= 0:
        raise ValueError("Binance ticker price was unavailable")
...
def _update_ohlc(pair: str, timeframe: str, rate: float, limit: int = 720) -> List[Dict[str, float]]:
    ...
    # Refresh exchange klines frequently so the currently-open candle moves like a real trading chart.
    fetch_every = 2 if timeframe in {"1m", "5m", "15m"} else 15
    ...
    cached_source = cached.get("source") or source or "internal"

    # For Binance-backed pairs, use Binance's own latest ticker/klines as the
    # live candle source. Do not use the animated market snapshot price here.
    # That animated movement is useful for custom/internal coins only and makes
    # real pairs look random if it drives the candle close.
    live_price = price
    if cached_source == "binance-klines":
        try:
            live_price = _fetch_binance_ticker_price_sync(normalized_pair)
        except Exception as exc:
            logger.debug("Unable to fetch Binance ticker for %s: %s", normalized_pair, exc)
            last_close = float((cached.get("candles") or [{}])[-1].get("close") or price)
            live_price = last_close

    live_candles = _apply_live_price_to_ohlc(cached.get("candles") or [], live_price, timeframe, limit)
    _ohlc_cache[key] = {"candles": live_candles, "source": cached_source, "last_fetch": cached.get("last_fetch") or now_ts}
...
@api.get("/trading/ohlc")
async def trading_ohlc(pair: str = "BTC/USDT", timeframe: str = "1m", limit: int = 320):
    base, quote = split_pair(pair)
    tf = timeframe if timeframe in OHLC_TIMEFRAME_SECONDS else "1m"
    ...
    display_rate = float(candles[-1].get("close") or rate) if candles else rate
    return {
        "pair": f"{base}/{quote}",
        "timeframe": tf,
        "rate": round(display_rate, 12),
        "candles": candles,
