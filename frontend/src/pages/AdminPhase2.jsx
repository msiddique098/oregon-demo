...
export function AdminGrowthEngine() {
    ...
    return <AdminLayout><div className="mb-8"><p className="text-xs uppercase tracking-widest text-amber-400/80">Task platform console</p><h1 className="text-3xl md:text-4xl font-display font-semibold mt-1">Growth Engine</h1></div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">{cards.map(([label, value, Icon]) => <Card key={label}><div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-widest text-zinc-500">{label}</p><p className="text-3xl font-display gradient-text-gold mt-2">{Number(value || 0).toLocaleString()}</p></div><div className="w-11 h-11 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center"><Icon className="w-5 h-5 text-amber-300" /></div></div></Card>)}</div></AdminLayout>;
}
...
export function AdminTaskSubmissions() {
    ...
    return <AdminLayout>
        <div className="mb-8"><p className="text-xs uppercase tracking-widest text-amber-400/80">Proof review queue</p><h1 className="text-3xl md:text-4xl font-display font-semibold mt-1">Task Submissions</h1><p className="text-zinc-400 mt-2">Review screenshot proof before user rewards are credited.</p></div>
        <div className="flex gap-2 flex-wrap mb-5">{["pending", "approved", "rejected", ""].map(f => <button key={f || "all"} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-xl border text-sm ${filter === f ? "bg-amber-500/10 border-amber-500/30 text-amber-200" : "bg-white/5 border-white/10 text-zinc-300"}`}>{f || "all"}</button>)}</div>
        ...
        {preview && <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setPreview(null)}><div className="max-w-5xl w-full" onClick={e => e.stopPropagation()}><div className="flex justify-between items-center mb-3"><h3 className="font-display text-xl">Proof preview</h3><button className="text-zinc-300" onClick={() => setPreview(null)}>✕</button></div><img src={preview.proof_data_url} alt="proof preview" className="max-h-[82vh] mx-auto rounded-xl border border-white/10" /></div></div>}
    </AdminLayout>;
}
