...
import { ArrowDownRight, ArrowUpRight, RefreshCcw, Search, Star, TrendingUp, Zap } from "lucide-react";
...
export default function Markets() {
    ...
    const filtered = useMemo(() => {
        ...
        return coins.filter((coin) => {
            ...
            return matchesSearch && (!onlyCustom || coin.custom);
        });
    }, [coins, search, onlyCustom]);

    const top = coins.filter((c) => !c.custom).slice(0, 4);
    const customCount = coins.filter((c) => c.custom).length;
        ...
        <DashboardLayout>
            <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-6">
                <div>
                    ...
                    <p className="text-zinc-400 mt-2 max-w-2xl">Top 200 market-cap coins with Eregon custom assets. Prices refresh automatically and can be used on the internal trading desk.</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    <Badge color={payload?.source === "coingecko" ? "emerald" : "gold"}>{payload?.provider || "Market source"}</Badge>
                    <Badge color="zinc">updates every 15s</Badge>
            ...
            <Card hover={false} className="mb-5">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div><h2 className="font-display text-xl">Spot Market</h2><p className="text-sm text-zinc-500">{filtered.length} assets visible, including {customCount} custom coins.</p></div>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                        ...
                        <button onClick={() => setOnlyCustom((v) => !v)} className={`btn-ghost px-4 ${onlyCustom ? "border-amber-400/30 text-amber-200" : ""}`}><Star className="w-4 h-4" /> Custom coins</button>
                    </div>
                </div>
            </Card>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {filtered.map((coin, idx) => {
                                ...
                                return <tr key={`${coin.id}-${coin.symbol}`} className="hover:bg-white/[0.03] transition-colors">
                                    <td className="px-4 py-4 text-zinc-500">{coin.custom ? <Zap className="w-4 h-4 text-amber-300" /> : coin.market_cap_rank || idx + 1}</td>
                                    <td className="px-4 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div><p className="font-semibold text-white">{coin.name}</p><p className="text-xs uppercase text-zinc-500">{coin.symbol}{coin.custom ? " · custom" : ""}</p></div>
                                        </div>
                                    </td>
