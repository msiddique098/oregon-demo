...
export default function Leaderboard() {
    ...
    return <DashboardLayout>
        ...
        {!rows ? <CinematicLoader /> : <div className="grid lg:grid-cols-3 gap-5">
            {rows.slice(0, 1).map((r) => <Card key={r.id} className="text-center lg:col-span-3"><div className="mx-auto w-14 h-14 rounded-2xl gradient-gold flex items-center justify-center neon-gold"><Medal className="w-7 h-7 text-black" /></div><p className="mt-4 text-xs text-zinc-500 uppercase tracking-widest">Approved community balance</p><h2 className="text-2xl font-display mt-1">{r.name}</h2><Badge color="gold" className="mt-3">{r.membership_name || "Member accounts"}</Badge><p className="text-3xl sm:text-4xl font-display gradient-text-gold mt-4">${Number(r.score || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p><p className="text-xs text-zinc-500 mt-3">Individual names, plans, join dates, and balances are hidden from member view.</p></Card>)}
