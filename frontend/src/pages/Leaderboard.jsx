...
export default function Leaderboard() {
    ...
    return <DashboardLayout>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 flex-wrap gap-3 mb-8">
            <div><p className="text-xs uppercase tracking-widest text-amber-400/80">Community competition</p><h1 className="text-2xl sm:text-3xl md:text-2xl sm:text-4xl font-display font-semibold mt-1">Royal Leaderboard</h1></div>
        ...
        {!rows ? <CinematicLoader /> : <div className="grid lg:grid-cols-3 gap-5">
            {rows.slice(0, 3).map((r, idx) => <Card key={r.id} className="text-center"><div className="mx-auto w-14 h-14 rounded-2xl gradient-gold flex items-center justify-center neon-gold"><Medal className="w-7 h-7 text-black" /></div><p className="mt-4 text-xs text-zinc-500 uppercase tracking-widest">Rank #{idx + 1}</p><h2 className="text-2xl font-display mt-1">{r.name}</h2><Badge color="gold" className="mt-3">{r.membership_name || "Free"}</Badge><p className="text-2xl sm:text-3xl font-display gradient-text-gold mt-4">{Number(r.score || 0).toLocaleString()}</p></Card>)}
            <div className="lg:col-span-3 glass-strong p-5 overflow-hidden"><div className="flex items-center gap-2 mb-4"><Trophy className="w-4 h-4 text-amber-300" /><p className="text-xs uppercase tracking-widest text-zinc-500">Live rankings</p></div><div className="space-y-2">{rows.map((r, idx) => <div key={r.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 rounded-xl bg-black/35 border border-white/5"><div className="flex items-center gap-3"><span className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-sm">{idx + 1}</span><div><p className="font-medium">{r.name}</p><p className="text-xs text-zinc-500">{r.membership_name || "Free"}</p></div></div><div className="flex items-center gap-2 text-amber-300"><Users className="w-4 h-4" /> {Number(r.score || 0).toLocaleString()}</div></div>)}</div></div>
        </div>}
