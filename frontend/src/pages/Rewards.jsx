...
function SpinWheel({ prizes, spinning, result }) {
    ...
    return <div className="relative mx-auto w-64 h-64 sm:w-72 sm:h-72 flex items-center justify-center">
        ...
        >
            ...
            {labels.map((p, i) => (
                <div key={`${p.label}-${i}`} className="absolute left-1/2 top-1/2 text-[10px] sm:text-xs font-semibold text-white/90 origin-left" style={{ transform: `rotate(${i * segment}deg) translate(54px, -50%)` }}>
                    <span className="inline-block -rotate-12 whitespace-nowrap">{p.label}</span>
                </div>
            ))}
            <div className="absolute inset-[36%] rounded-full gradient-gold flex items-center justify-center text-black font-display font-bold">SPIN</div>
        </motion.div>
    ...
    return <DashboardLayout>
            ...
            <Card className="shadow-[0_0_35px_rgba(251,191,36,.15)]">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <h2 className="text-2xl font-display mt-2">Today: <span className="gradient-text-gold">${nextCheckin.toFixed(2)}</span></h2>
                        <p className="text-sm text-zinc-400 mt-2">One claim per day. Sequence: $2, $4, ... $20, then $18, $16 ... back to $2.</p>
                    </div>
                    <div className="w-12 h-12 rounded-2xl gradient-gold flex items-center justify-center neon-gold"><Gift className="w-6 h-6 text-black" /></div>
                </div>
                <button onClick={checkin} className="btn-gold w-full mt-5 sticky bottom-24 md:static"><Sparkles className="w-4 h-4" /> Claim check-in</button>
            </Card>
            ...
            <Card className="relative overflow-hidden border-amber-400/20 shadow-[0_0_35px_rgba(251,191,36,.12)]">
                ...
                <div className="flex items-start justify-between gap-4 relative z-10">
                    <div>
                        ...
                        <h2 className="text-2xl font-display mt-2">Get <span className="gradient-text-gold">30% extra bonus</span></h2>
                        <p className="text-sm text-zinc-400 mt-2">Approved deposits add to your balance and target progress with an additional 30% platform bonus. Admin can also unlock extra spin access with approved plans or deposits.</p>
                    </div>
                    <Zap className="w-8 h-8 text-amber-300" />
                </div>
                <button onClick={() => navigate('/dashboard/deposit')} className="btn-gold w-full mt-5 relative z-10">Deposit & Unlock Bonus <ArrowRight className="w-4 h-4" /></button>
            </Card>
        </div>
            ...
            <Card className="lg:col-span-1">
                <div className="flex items-start justify-between gap-4 mb-4">
                    <div>
                        ...
                        <h2 className="text-2xl font-display mt-2">Reward Spin Wheel</h2>
                        <p className="text-sm text-zinc-400 mt-2">Available spins: {overview?.spin_tokens || 0}. Plans and approved deposits can unlock additional spin access.</p>
                    </div>
                    <RotateCcw className="w-6 h-6 text-amber-300" />
                </div>
                <SpinWheel prizes={prizes} spinning={spinning} result={spinResult} />
                <button disabled={spinning || Number(overview?.spin_tokens || 0) <= 0} onClick={spin} className={`btn-royal w-full mt-14 ${spinning || Number(overview?.spin_tokens || 0) <= 0 ? "opacity-60 cursor-not-allowed" : ""}`}><Zap className="w-4 h-4" /> {spinning ? "Spinning..." : "Spin now"}</button>
            </Card>
        ...
        <div className="grid lg:grid-cols-2 gap-5">
            ...
            <Card>
                ...
                <p className="text-sm text-zinc-400 mt-2">Required balance: {nextVip?.required_balance || 0} USDT</p>
